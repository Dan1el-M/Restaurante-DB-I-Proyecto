...
class Table(Base):
    ...
    table_status = Column(Integer, nullable=False, default=1)  # 1 = disponible, 0 = ocupada, 2 = reservada
    ...
    # Restricciones
    # No se repita el numero de mesa
    __table_args__ = (
        UniqueConstraint('restaurant_id', 'table_number', name='unique_table_per_restaurant'),
    )
    
    def __repr__(self):
        return f"<Table(table_id={self.table_id}, table_number={self.table_number}, table_status={self.table_status})>"